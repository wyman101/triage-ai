...
import { type MergedResult, type FindingCluster, clusterRepresentative, isConsensus } from './types.js';
...
export async function clearMemory(
  ...
  for (const model of targetModels) {
    ...
    try {
      ...
      if (!existing.includes(MEMORY_START)) continue;

      if (!existing.includes(MEMORY_END)) {
        console.log(
          `  Warning: ${filename} has start marker but no end marker — skipping`,
        );
        continue;
      }
...
export function _buildMemoryContent(merged: MergedResult, prompt: string): string {
  const total =
  ...
  lines.push('');
  lines.push(
    `*Last run: ${timestamp} — ${total} issues found, ${merged.consensus.length} consensus*`,
  );
  lines.push('');
  ...
  if (merged.medium.length > 0) {
    ...
    for (const cluster of merged.medium) {
      ...
      lines.push(
        `- [${f.severity}] ${f.title} — \`${f.location.path}:${f.location.start_line}\`${consensusTag}`,
      );
    }
    lines.push('');
  }
...
export function _formatCluster(cluster: FindingCluster): string[] {
  const lines: string[] = [];
  const f = clusterRepresentative(cluster);
  const consensusTag = isConsensus(cluster) ? ' **(consensus)**' : '';
  ...
  lines.push(
    `  - Location: \`${f.location.path}:${f.location.start_line}-${f.location.end_line}\``,
  );
  lines.push(`  - Models: ${models}`);

  if (f.recommendation) {
    let rec = f.recommendation.slice(0, 200);
    if (f.recommendation.length > 200) rec += '...';
    lines.push(`  - Fix: ${rec}`);
  }
 ...
 */
export async function _updateMemoryFile(
  filepath: string,
  content: string,
): Promise<boolean> {
  try {
