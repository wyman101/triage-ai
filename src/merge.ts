...
export class MergeEngine {
  ...
  merge(results: ModelResult[]): MergedResult {
    ...
    for (const result of results) {
      ...
      for (const finding of result.findings) {
        ...
        const f: Finding = { ...finding, model: result.model };
        allFindings.push(f);
      }
    }
    ...
    const byModelCount = (a: FindingCluster, b: FindingCluster) =>
  ...
  _detectConflicts(clusters: FindingCluster[]): Conflict[] {
    ...
    const sevValues: Record<string, number> = { S0: 0, S1: 1, S2: 2, S3: 3 };

    for (const cluster of clusters) {
      if (cluster.findings.length < 2) continue;
...
function clusterToDict(cluster: FindingCluster): Record<string, unknown> {
  const rep = clusterRepresentative(cluster);
  return {
...
export function mergedResultToDict(merged: MergedResult): Record<string, unknown> {
  return {
    ...
    unique_by_model: Object.fromEntries(
      Object.entries(merged.unique_by_model).map(([model, findings]) => [
        model,
        findings.map((f) => ({
          title: f.title,
          severity: f.severity,
