...
function _authHint(modelName: string, errorMsg: string): string {
  ...
  if (name === 'codex')  return 'not authenticated — set OPENAI_API_KEY env variable';
...
const KNOWN_TOOLS: Omit<CliTool, 'path' | 'available'>[] = [
  {
    name: 'Claude',
    command: 'claude',
...
async function detectTools(requested: string[]): Promise<CliTool[]> {
  const names = new Set(requested.map((n) => n.trim().toLowerCase()));
  const tools: CliTool[] = [];

  for (const known of KNOWN_TOOLS) {
    if (!names.has(known.command.toLowerCase())) continue;
...
async function loadModel(modelName: string): Promise<BaseModel> {
...
function writeMemory(merged: MergedResult, prompt: string): void {
  ...
  for (const cluster of allFindings.slice(0, 20)) {
    ...
    lines.push(
      `- **[${rep?.severity ?? 'S3'}]** ${rep?.title ?? 'Unknown'} ` +
...
async function runSetup(): Promise<void> {
  console.log('\ntriage-ai setup\n');
  console.log('Checking for available AI CLI tools...\n');

  const tools = await detectTools(['claude', 'gemini', 'codex']);
  for (const t of tools) {
    const status = t.available
      ? `  ✓ ${t.name} found at ${t.path}`
      : `  ✗ ${t.name} not installed — ${t.install_cmd}`;
    console.log(status);
  }
  ...
  if (available.length === 0) {
    console.log(
      '\nNo AI CLI tools found.  Please install at least one and re-run setup.',
    );
    return;
  }

  const defaultModels = available.map((t) => t.command).join(',');
  ...
  if (process.stdin.isTTY) {
    ...
    console.log('');
    const modelsAnswer = await ask(`Default models to use [${defaultModels}]: `);
    models = modelsAnswer.trim() || defaultModels;
    rl.close();
  } else {
    console.log(`\nUsing default models: ${defaultModels}`);
  }

  const config = {
    models,
    last_setup: new Date().toISOString(),
    cli_paths: Object.fromEntries(
      available.filter((t) => t.path).map((t) => [t.command, t.path as string]),
    ),
  };

  writeConfig(config);
  console.log(`\nConfiguration saved to ${CONFIG_FILE}`);
  console.log('\nYou can now run: triage-ai "your analysis prompt"');
}
...
async function main(): Promise<void> {
  ...
  progress.stopSpinner(
    'Scanning repository',
    'done',
    `${fileCount} files${diffNote}`,
  );

  // Secret redaction count if the scanner exposes it
  const redactedCount = (context as ScanContext & { redacted_count?: number }).redacted_count ?? 0;
  if (redactedCount > 0) {
  ...
  progress.updateItem(
    'Clustered findings',
    'done',
    `${totalFindings} unique issues from ${successResults.length} model${successResults.length === 1 ? '' : 's'}`,
  );
  ...
  if (merged.consensus.length > 0) {
    ...
    progress.updateItem(
      'Consensus detected',
      'done',
      `${merged.consensus.length} issue${merged.consensus.length === 1 ? '' : 's'} confirmed by 2+ models`,
    );
  }
  ...
  if (merged.conflicts.length > 0) {
    ...
    progress.updateItem(
      ...
      'done',
      `${merged.conflicts.length} severity disagreement${merged.conflicts.length === 1 ? '' : 's'}`,
    );
  }

  if (!process.stdout.isTTY) {
    process.stdout.write(
      `[diag] ${totalFindings} issues, ${merged.consensus.length} consensus, ` +
  ...
  if (opts.out) {
    ...
    progress.updateItem(
      'Generated report',
      'done',
      `${merged.blockers.length} blockers, ${merged.high.length} high, ` +
