...
export class ReportGenerator {
  ...
  toMarkdown(
    ...
    // ---- Model Run Summary Table ----
    if (merged.model_runs.length > 0) {
      lines.push('### Model Run Summary');
      lines.push('');
      lines.push('| Model | Status | Findings | Duration | Format |');
      lines.push('| :--- | :--- | :--- | :--- | :--- |');
      for (const run of merged.model_runs) {
        const dur = run.elapsed_ms ? `${(run.elapsed_ms / 1000).toFixed(1)}s` : '-';
        const fmt = run.parsed_as === 'plain_text' ? 'prose' : run.parsed_as ?? '-';
        const statusStr = run.status === 'succeeded' ? '✓' : run.status === 'failed' ? '✗' : run.status;
        lines.push(`| ${run.model} | ${statusStr} | ${run.findings_count} | ${dur} | ${fmt} |`);
      }
      lines.push('');
    ...
    if (merged.consensus.length > 0) {
      ...
      for (const cluster of merged.consensus) {
        ...
        lines.push(
          `- [${f.severity}] **${f.title}** (${f.location.path}) - *${models}*`,
        );
      }
      lines.push('');
    }
    ...
    if (planItems.length > 0) {
      ...
      lines.push(
        'No critical issues found. Consider reviewing medium/low findings.',
      );
    }

    lines.push('');
    lines.push('---');
    lines.push(`*Report generated by triage-ai v${VERSION}*`);
