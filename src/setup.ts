 ...
 */
export async function detectClis(): Promise<CliTool[]> {
 ...
 */
export async function loadConfig(): Promise<TriageConfig | null> {
  try {
 ...
 */
export async function runSetup(): Promise<CliTool[]> {
  ...
  console.log(
    `${'Tool'.padEnd(nameWidth)}  ${'Status'.padEnd(statusWidth)}  Detail`,
  );
  console.log(
    `${'-'.repeat(nameWidth)}  ${'-'.repeat(statusWidth)}  ${'-'.repeat(40)}`,
  );
 ...
 */
export async function ensureCliAvailable(modelName: string): Promise<CliTool | null> {
 ...
 */
export async function getAvailableModels(requestedModels: string[]): Promise<CliTool[]> {
  const allTools = await detectClis();
  const toolMap = new Map<string, CliTool>(allTools.map((t) => [t.name, t]));
  ...
  for (const name of requestedModels) {
    ...
    if (!tool) {
      process.stderr.write(
        `triage-ai: warning: unknown model "${name}" — valid options are: ${CLI_TOOLS.map((t) => t.name).join(', ')}\n`,
      );
      continue;
    }
    if (!tool.available) {
      process.stderr.write(
        `triage-ai: warning: "${name}" is not installed or not found in PATH.\n` +
