...
const TriageInputSchema = {
    prompt: z.string().describe('What to analyze (e.g., "find security vulnerabilities in the authentication flow")'),
    models: z
        .string()
        .optional()
 ...
 */
function pruneResultsDirs() {
    if (!pathExists(RESULTS_BASE))
        return;
    let entries = [];
    try {
        entries = readdirSync(RESULTS_BASE)
            ...
            .filter((e) => e !== null && isDirectory(e.path))
 ...
 */
function toMarkdown(merged, prompt, elapsedSec, modelNames) {
...
function writeMemory(merged, prompt) {
    ...
    for (const cluster of allFindings.slice(0, 20)) {
        ...
        if (!rep)
            continue;
        const models = [...cluster.models].join(', ');
        lines.push(`- **[${rep.severity}]** ${rep.title} ` +
...
function createServer() {
    ...
    // Tool: triage
    // -------------------------------------------------------------------------
    server.tool('triage', 'Run multi-model code triage using Claude, Gemini and Codex in parallel. ' +
        'Analyzes code for bugs, security issues, performance problems and more. ' +
        'Models run concurrently and findings are merged with consensus detection.', TriageInputSchema, async (args) => {
        const { prompt, models: modelsArg = 'claude,gemini,codex', diff_only = false, max_files = 30, format = 'md', timeout = 300, remember = false, } = args;
        if (!prompt?.trim()) {
            return {
                content: [{ type: 'text', text: 'Error: No prompt provided' }],
                isError: true,
            };
        }
        ...
        try {
            ...
            return {
                content: [{ type: 'text', text: `Error scanning repository: ${msg}` }],
                isError: true,
            };
        }
        ...
        const modelPairs = await Promise.all(modelNames.map(async (name) => ({ name, model: await loadModel(name) })));
        const validPairs = modelPairs.filter((p) => p.model !== null);
        ...
        const taskResults = await Promise.allSettled(validPairs.map(async ({ name, model }) => {
            try {
                ...
                const msg = err instanceof Error ? err.message : String(err);
                return { name, result: null, error: msg };
            }
        }));
        ...
        if (successResults.length === 0) {
            ...
            return {
                content: [
                    { type: 'text', text: `Error: All models failed — ${detail}` },
                ],
                isError: true,
            };
        }
        ...
        await writeFile(mergedPath, JSON.stringify(mergedResultToDict(merged), null, 2), 'utf8');
        return {
